...
abstract class ElevationItemTouchHelperCallback(private val activeElevationChange: Float, dragDirs: Int = 0, swipeDirs: Int = 0) :
    ...
    private fun updateElevation(recyclerView: RecyclerView, holder: RecyclerView.ViewHolder, elevate: Boolean) {
        if (elevate) {
            ...
            ObjectAnimator.ofFloat(holder.itemView, "Elevation", originalElevation, activeElevationChange + findMaxElevation(recyclerView)).apply {
                duration = 150
            }.start()
            isElevated = true
        } else {
            ObjectAnimator.ofFloat(holder.itemView, "Elevation", ViewCompat.getElevation(holder.itemView), originalElevation).apply {
                duration = 150
            }.start()
